...
from src.types.voxradar_news_scrapping_opovo_noticias_queue_dto import VoxradarNewsScrappingOpovoNoticiasQueueDTO
...
class ScrappingNewsOpovoNoticiasService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News O Povo Noticias Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        otempo_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_opovonoticias_queue_dto:VoxradarNewsScrappingOpovoNoticiasQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_opovonoticias_queue_dto.url
...
#           
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingOpovoNoticiasQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingOpovoNoticiasQueueDTO(body.get('url'))
