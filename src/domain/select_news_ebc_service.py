...
class SelectNewsEbcService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Ebc | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.ebc.com.br/sala-de-imprensa/noticias/rss"
        
        try:
            links_filtered = Utils.extract_links_from_rss_ebc(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site ebc | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingEbcQueueDTO = VoxradarNewsScrappingEbcQueueDTO(url)
