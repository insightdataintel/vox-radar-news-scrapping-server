...
class SelectNewsMaisGoiasService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Mais Goias Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.maisgoias.com.br/ultimas-noticias/"
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingMaisGoiasQueueDTO = VoxradarNewsScrappingMaisGoiasQueueDTO(url)
