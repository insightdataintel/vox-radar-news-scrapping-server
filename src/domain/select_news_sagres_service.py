...
class SelectNewsSagresService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Sagres Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://sagresonline.com.br/ultimas/"

        
        try:
            links_filtered = Utils.extract_links_from_page_sagres(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site sagres | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingSagresQueueDTO = VoxradarNewsScrappingSagresQueueDTO(url)
