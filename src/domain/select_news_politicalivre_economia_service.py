...
class SelectNewsPoliticalivreEconomiaService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Politica Livre | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://politicalivre.com.br/category/economia"
        
        try:
            links_filtered = Utils.extract_links_from_page_politicalivre(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site politicalivre economia | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingPoliticalivreEconomiaQueueDTO = VoxradarNewsScrappingPoliticalivreEconomiaQueueDTO(url)
