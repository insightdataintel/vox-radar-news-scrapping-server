...
class SelectNewsARedacaoService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News A Redação Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://aredacao.com.br/ultimas/"
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingARedacaoQueueDTO = VoxradarNewsScrappingARedacaoQueueDTO(url)
