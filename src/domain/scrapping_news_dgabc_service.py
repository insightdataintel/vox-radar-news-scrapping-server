...
from src.types.voxradar_news_scrapping_dgabc_queue_dto import VoxradarNewsScrappingDgabcQueueDTO
...
class ScrappingNewsDgabcService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        ...
        dgabc_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_dgabc_queue_dto:VoxradarNewsScrappingDgabcQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_dgabc_queue_dto.url
    ...
    #
        try:
            ...
            title = str(title).split('content="')[1].split('- Diário do Grande ABC')[0]
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar o título da notícia do DGABC: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do DGABC: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do DGABC: {url_news} | {e}")
            return ReturnService(False, 'Did not collect the body of the News')    
    #    
    # Pick category news
    #   
        category_news = soup.find('div', class_ = 'linha1')
        category_news = str(category_news).split('target="_self">')[1].split('</a>')[0].strip().lower()
    ...
    # Pick image from news
        ...
        #
        #
        dgabc_dict["title"].append(title)
        dgabc_dict["domain"].append(domain)
        dgabc_dict["source"].append(source)
        dgabc_dict["date"].append(date)
        dgabc_dict["body_news"].append(body_new)
        dgabc_dict["link"].append(url_news)
        dgabc_dict["category"].append(category_news)
        dgabc_dict["image"].append(image_new)
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingDgabcQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingDgabcQueueDTO(body.get('url'))
