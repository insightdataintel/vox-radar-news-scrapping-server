...
from src.types.voxradar_news_scrapping_noticias_concursos_queue_dto import VoxradarNewsScrappingNoticiasConcursosQueueDTO
...
class ScrappingNewsNoticiasConcursoservice(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Noticias Concursos Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        noticiasconcursos_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_noticiasconcursos_queue_dto:VoxradarNewsScrappingNoticiasConcursosQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_noticiasconcursos_queue_dto.url
    ...
    #
        try:
            ...
            mode = ['div']
            classk = ['col-sm-8 content-column']
            paragraf = ['p']
            body_new = ''
    ...
    # Pick category news
    #   
        category_news = soup.find_all("script", type="text/javascript")
        category_news = str(category_news).split('pageCategory":')[1].split('datatype')[0].split(',"pageAttributes"')[0].split('<span')[0].replace('"','')\
            .replace('[', '').replace(']', '')
    ...
    # Pick image from news
        ...
        #
        #
        noticiasconcursos_dict["title"].append(title)
        noticiasconcursos_dict["domain"].append(domain)
        noticiasconcursos_dict["source"].append(source)
        noticiasconcursos_dict["date"].append(date)
        noticiasconcursos_dict["body_news"].append(body_new)
        noticiasconcursos_dict["link"].append(url_news)
        noticiasconcursos_dict["category"].append(category_news)
        noticiasconcursos_dict["image"].append(image_new)
        

        print(noticiasconcursos_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingNoticiasConcursosQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingNoticiasConcursosQueueDTO(body.get('url'))
