...
class SelectNewsTerraNoticiasService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Terra Noticia | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site terra noticia | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingTerraNoticiasQueueDTO = VoxradarNewsScrappingTerraNoticiasQueueDTO(url)
