...
class SelectNewsPoliarquiaService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Poliarquia | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://poliarquia.com.br/category/noticias/"
        
        try:
            links_filtered = Utils.extract_links_from_page_poliarquia(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site poliarquia | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingPoliarquiaQueueDTO = VoxradarNewsScrappingPoliarquiaQueueDTO(url)
