...
from src.types.voxradar_news_scrapping_em_economia_queue_dto import VoxradarNewsScrappingEmEconomiaQueueDTO
...
class ScrappingNewsEmEconomiaService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Em Economia Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        em_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_emeconomia_queue_dto:VoxradarNewsScrappingEmEconomiaQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_emeconomia_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Em Economia: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        ...
        #
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Em Economia: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Em Economia: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Em Economia: {url_news} | {e}")     
            image_new = "" 
        # #
        # #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingEmEconomiaQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingEmEconomiaQueueDTO(body.get('url'))
