...
class SelectNewsOHojeService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News O Hoje Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site ohoje | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingOHojeQueueDTO = VoxradarNewsScrappingOHojeQueueDTO(url)
