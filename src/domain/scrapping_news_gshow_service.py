...
from src.types.voxradar_news_scrapping_gshow_queue_dto import VoxradarNewsScrappingGShowQueueDTO
...
class ScrappingNewsGShowService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Globo Show Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        globo_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_gshow_queue_dto:VoxradarNewsScrappingGShowQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_gshow_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do GShow: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do GShow: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do GShow: {url_news} | {e}")
            body_new = ""
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do GShow: {url_news} | {e}")     
            image_new = "" 
        #
        #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingGShowQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingGShowQueueDTO(body.get('url'))
