...
class SelectNewsNdmaisService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Ndmais | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            links_filtered = Utils.extract_links_from_page_ndmais(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site ndmais | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingNdmaisQueueDTO = VoxradarNewsScrappingNdmaisQueueDTO(url)
