...
class SelectNewsExameService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Exame | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://exame.com/ultimas-noticias/"
        
        try:
            links_filtered = Utils.extract_links_from_page_exame(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site exame | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingExameQueueDTO = VoxradarNewsScrappingExameQueueDTO(url)
