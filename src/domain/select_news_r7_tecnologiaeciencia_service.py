...
class SelectNewsR7TecnologiaecienciaService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News R7 Tecnologia e Ciência Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site r7 tecnologiaeciencia | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingR7TecnologiaecienciaQueueDTO = VoxradarNewsScrappingR7TecnologiaecienciaQueueDTO(url)
