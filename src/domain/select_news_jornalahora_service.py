...
class SelectNewsJornalAHoraService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Jornal A Hora Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://jornalahora.com/"

        
        try:
            links_filtered = Utils.extract_links_from_page_jornalahora(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site jornalahora | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingJornalAHoraQueueDTO = VoxradarNewsScrappingJornalAHoraQueueDTO(url)
