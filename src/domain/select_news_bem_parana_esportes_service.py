...
class SelectNewsBemParanaEsportesService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Bem Parana Esportes | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.bemparana.com.br/esportes/"
        
        try:
            links_filtered = Utils.extract_links_from_page_bemparana(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site Bem Parana Esportes | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingBemParanaEsportesQueueDTO = VoxradarNewsScrappingBemParanaEsportesQueueDTO(url)
