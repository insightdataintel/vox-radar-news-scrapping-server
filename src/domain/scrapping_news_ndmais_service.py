...
from src.types.voxradar_news_scrapping_ndmais_queue_dto import VoxradarNewsScrappingNdmaisQueueDTO
...
class ScrappingNewsNdmaisService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        ...
        ndmais_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_ndmais_queue_dto:VoxradarNewsScrappingNdmaisQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_ndmais_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do ND Mais: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do ND Mais: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do ND Mais: {url_news} | {e}")
    ...
    # 
        ...
        category_news = str(category_news).split('content=')[1].split('property=')[0].replace('"','').lower()
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do ND Mais: {url_news} | {e}")     
            image_new = "" 
        # #
        # #
        ...
        #
        #
        ndmais_dict["title"].append(title)
        ndmais_dict["domain"].append(domain)
        ndmais_dict["source"].append(source)
        ndmais_dict["date"].append(date)
        ndmais_dict["body_news"].append(body_new)
        ndmais_dict["link"].append(url_news)
        ndmais_dict["category"].append(category_news)
        ndmais_dict["image"].append(image_new)
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingNdmaisQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingNdmaisQueueDTO(body.get('url'))
