...
class SelectNewsGloboValorService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Globo Valor Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "http://pox.globo.com/rss/valor"
        
        try:
            links_filtered = Utils.extract_links_from_rss(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site globo valor | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingGloboValorQueueDTO = VoxradarNewsScrappingGloboValorQueueDTO(url)
