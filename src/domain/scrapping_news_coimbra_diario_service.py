...
from src.types.voxradar_news_scrapping_coimbra_diario_queue_dto import VoxradarNewsScrappingCoimbraDiarioQueueDTO
...
class ScrappingNewsCoimbraDiarioService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Coimbra Diario Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        coimbradiario_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_coimbradiario_queue_dto:VoxradarNewsScrappingCoimbraDiarioQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_coimbradiario_queue_dto.url
    ...
    #
        try:
            ...
            title = str(title).split("<title>")[1].split("– Coimbra Diario</title>")[0].split('itemprop=')[0].split('- ISTOÉ DINHEIRO')[0].split('| Exame')[0].replace('- @aredacao','').replace('"','')
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar o título da notícia do Coimbra Diario: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            date = soup.find_all("span", class_="entry-date post-date")
            date = str(date).split('title="')[1].split('">')[0].split(',')[0].split('<span')[0].replace('T', ' ').replace('Z', '').\
            ...
            date = "%s-3:00"%(str(date.strftime('%Y-%m-%d %H:%M:%S')))
                ...
                self.logger.error(f"Não foi possível encontrar a data da notícia do Coimbra Diario: {url_news} | {e}")
                date = ""    
    #
    #Pick body's news
    #
    ...
    # Pick category news
    #   
        category_news = soup.find("span", class_="post-category")
        category_news = str(category_news).split('</i>')[1].split('datatype')[0].split(',"pageAttributes"')[0].split('</span')[0].replace('"','')\
    ...
    # Pick image from news
        ...
        try:
            source = url_news.split("http://")[1].split(".com")[0]               
        except:
            source = url_news.split("https://")[1].split(".com")[0]               
               
        #
        #
        coimbradiario_dict["title"].append(title)
        coimbradiario_dict["domain"].append(domain)
        coimbradiario_dict["source"].append(source)
        coimbradiario_dict["date"].append(date)
        coimbradiario_dict["body_news"].append(body_new)
        coimbradiario_dict["link"].append(url_news)
        coimbradiario_dict["category"].append(category_news)
        coimbradiario_dict["image"].append(image_new)
        

        print(coimbradiario_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingCoimbraDiarioQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingCoimbraDiarioQueueDTO(body.get('url'))
