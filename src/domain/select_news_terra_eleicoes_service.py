...
class SelectNewsTerraEleicoesService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Terra Eleições | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.terra.com.br/noticias/eleicoes/"
        
        try:
            links_filtered = Utils.extract_links_from_page_terra(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site terra eleicoes | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingTerraEleicoesQueueDTO = VoxradarNewsScrappingTerraEleicoesQueueDTO(url)
