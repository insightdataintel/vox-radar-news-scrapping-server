...
from src.types.voxradar_news_scrapping_em_educacao_queue_dto import VoxradarNewsScrappingEmEducacaoQueueDTO
...
class ScrappingNewsEmEducacaoService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Em Educacao Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        em_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_emeducacao_queue_dto:VoxradarNewsScrappingEmEducacaoQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_emeducacao_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Em Educacao: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Em Educacao: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Em Educacao: {url_news} | {e}")     
            image_new = "" 
        # #
        # #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingEmEducacaoQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingEmEducacaoQueueDTO(body.get('url'))
