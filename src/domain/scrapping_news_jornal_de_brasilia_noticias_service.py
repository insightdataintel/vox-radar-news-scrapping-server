...
from src.types.voxradar_news_scrapping_jornal_de_brasilia_noticias_queue_dto import VoxradarNewsScrappingJornaldeBrasiliaNoticiasQueueDTO
...
class ScrappingNewsJornaldeBrasiliaNoticiasService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Jornal de Brasilia Noticias Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        jornaldebrasilia_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_jornaldebrasilianoticias_queue_dto:VoxradarNewsScrappingJornaldeBrasiliaNoticiasQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_jornaldebrasilianoticias_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Jornal de Brasilia Noticias: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Jornal de Brasilia Noticias: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Jornal de Brasilia Noticias: {url_news} | {e}")
            return ReturnService(False, 'Did not collect the body of the News')    
    #    
    # Pick category news
    #   
        category_news = url_news.replace("https://",'').split('/')[2]
    ...
    # Pick image from news
        ...
        #
        #
        jornaldebrasilia_dict["title"].append(title)
        jornaldebrasilia_dict["domain"].append(domain)
        jornaldebrasilia_dict["source"].append(source)
        jornaldebrasilia_dict["date"].append(date)
        jornaldebrasilia_dict["body_news"].append(body_new)
        jornaldebrasilia_dict["link"].append(url_news)
        jornaldebrasilia_dict["category"].append(category_news)
        jornaldebrasilia_dict["image"].append(image_new)
        

        print(jornaldebrasilia_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingJornaldeBrasiliaNoticiasQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingJornaldeBrasiliaNoticiasQueueDTO(body.get('url'))
