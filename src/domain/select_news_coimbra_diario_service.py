...
class SelectNewsCoimbraDiarioService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Coimbra Diario | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "http://coimbradiario.com/"
        
        try:
            links_filtered = Utils.extract_links_from_page_coimbradiario(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site coimbra diario | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingCoimbraDiarioQueueDTO = VoxradarNewsScrappingCoimbraDiarioQueueDTO(url)
