...
class SelectNewsCNNBrasilEsporteService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        ...
        url = "https://www.cnnbrasil.com.br/esporte/"
        
        try:
            links_filtered = Utils.extract_links_from_page_cnnbrasil(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site cnn brasil esporte | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingCNNBrasilEsporteQueueDTO = VoxradarNewsScrappingCNNBrasilEsporteQueueDTO(url)
