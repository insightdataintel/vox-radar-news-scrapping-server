...
class SelectNewsReporterDiarioEsportesService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        ...
        url = "https://www.reporterdiario.com.br/esportes/"
        
        try:
            links_filtered = Utils.extract_links_from_page_reporterdiario(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site reporter diario esportes | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingReporterDiarioEsportesQueueDTO = VoxradarNewsScrappingReporterDiarioEsportesQueueDTO(url)
