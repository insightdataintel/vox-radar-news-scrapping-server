...
from src.types.voxradar_news_scrapping_istoe_dinheiro_queue_dto import VoxradarNewsScrappingIstoeDinheiroQueueDTO
...
class ScrappingNewsIstoeDinheiroService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        ...
        istoedinheiro_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_istoe_dinheiro_queue_dto:VoxradarNewsScrappingIstoeDinheiroQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_istoe_dinheiro_queue_dto.url
    ...
    # Pick image from news
        ...
        #
        #
        istoedinheiro_dict["title"].append(title)
        istoedinheiro_dict["domain"].append(domain)
        istoedinheiro_dict["source"].append(source)
        istoedinheiro_dict["date"].append(date)
        istoedinheiro_dict["body_news"].append(body_new)
        istoedinheiro_dict["link"].append(url_news)
        istoedinheiro_dict["category"].append(category_news)
        istoedinheiro_dict["image"].append(image_new)
        

        print(istoedinheiro_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingIstoeDinheiroQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingIstoeDinheiroQueueDTO(body.get('url'))
