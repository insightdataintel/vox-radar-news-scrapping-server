...
from src.types.voxradar_news_scrapping_r7_educacao_queue_dto import VoxradarNewsScrappingR7EducacaoQueueDTO
...
class ScrappingNewsR7EducacaoService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News R7 Educacao Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        r7_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_r7_educacao_queue_dto:VoxradarNewsScrappingR7EducacaoQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_r7_educacao_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do R7 educacao: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do R7 educacao: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
# 
        try:     
            body_new = ''
            mode = ['article', 'div']
            classk = ['toolkit-media-content','container','offset-md-2 col-md-8 postagem','entry-content materia','entry-content','single-container','texto',\
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do R7 educacao: {url_news} | {e}")
            return ReturnService(False, 'Did not collect the body of the News')

    # Pick category news
    # 
        category_news = 'educação'
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do R7 educacao: {url_news} | {e}")     
            image_new = ""
        #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingR7EducacaoQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingR7EducacaoQueueDTO(body.get('url'))
