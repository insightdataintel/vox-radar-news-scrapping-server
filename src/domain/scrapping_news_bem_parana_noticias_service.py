...
from src.types.voxradar_news_scrapping_bem_parana_noticias_queue_dto import VoxradarNewsScrappingBemParanaNoticiasQueueDTO
...
class ScrappingNewsBemParanaNoticiasService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Bem Parana Noticias Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        bemparana_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_bemparana_queue_dto:VoxradarNewsScrappingBemParanaNoticiasQueueDTO = self.__parse_body(body)
    ...
    #  
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Bem Parana Noticias: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Bem Parana Noticias: {url_news} | {e}")
            date = ""
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Bem Parana Noticias: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Bem Parana Noticias: {url_news} | {e}")     
            image_new = ""      
        #
        #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingBemParanaNoticiasQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingBemParanaNoticiasQueueDTO(body.get('url'))
