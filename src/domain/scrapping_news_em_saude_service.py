...
from src.types.voxradar_news_scrapping_em_saude_queue_dto import VoxradarNewsScrappingEmSaudeQueueDTO
...
class ScrappingNewsEmSaudeService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Em Saude Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        em_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_emsaude_queue_dto:VoxradarNewsScrappingEmSaudeQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_emsaude_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Em Saude: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Em Saude: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Em Saude: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Em Saude: {url_news} | {e}")     
            image_new = ""
        # #
        # #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingEmSaudeQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingEmSaudeQueueDTO(body.get('url'))
