...
class SelectNewsGloboService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Globo Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            links_filtered = Utils.extract_links_from_page_globo(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site globo | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingGloboQueueDTO = VoxradarNewsScrappingGloboQueueDTO(url)
