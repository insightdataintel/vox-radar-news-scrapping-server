...
class SelectNewsOpovoNoticiasService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News O Povo Notícias Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.opovo.com.br/noticias"
        
        try:
            links_filtered = Utils.extract_links_from_page(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site opovonoticias | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingOpovoNoticiasQueueDTO = VoxradarNewsScrappingOpovoNoticiasQueueDTO(url)
