...
class SelectNewsIstoeDinheiroService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Istoe Dinheiro | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.istoedinheiro.com.br/ultimas/"
        
        try:
            links_filtered = Utils.extract_links_from_page_istoe_dinheiro(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site istoe dinheiro | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingIstoeDinheiroQueueDTO = VoxradarNewsScrappingIstoeDinheiroQueueDTO(url)
