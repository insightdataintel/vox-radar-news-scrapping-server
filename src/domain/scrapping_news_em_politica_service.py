...
class ScrappingNewsEmPoliticaService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Em Politica Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        em_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_empolitica_queue_dto:VoxradarNewsScrappingEmPoliticaQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_empolitica_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Em Politica: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        ...
        #
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Em Politica: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Em Politica: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Em Politica: {url_news} | {e}")     
            image_new = "" 
        # #
        # #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingEmPoliticaQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingEmPoliticaQueueDTO(body.get('url'))
