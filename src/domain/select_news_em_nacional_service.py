...
class SelectNewsEmNacionalService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Em Nacional | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site em naciional | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingEmNacionalQueueDTO = VoxradarNewsScrappingEmNacionalQueueDTO(url)
