...
from src.types.voxradar_news_scrapping_ig_queue_dto import VoxradarNewsScrappingIgQueueDTO
...
class ScrappingNewsIgService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        ...
        ig_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_ig_queue_dto:VoxradarNewsScrappingIgQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_ig_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do IG: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            date = str(date).split('content="')[1].split('itemprop=')[0].replace('T', ' ').replace('Z', '').replace('"','').strip()
        ...
        #
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do IG: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do IG: {url_news} | {e}")
            body_new = ""

    # Pick category news
    # 
        category_news = soup.find("meta", attrs={'name': 'category'})
        category_news = str(category_news).split('content="')[1].split('name=')[0].replace('"','').replace('/','').strip()
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do IG: {url_news} | {e}")     
            image_new = "" 
        #
        #
        ...
        #
        #
        ig_dict["title"].append(title)
        ig_dict["domain"].append(domain)
        ig_dict["source"].append(source)
        ig_dict["date"].append(date)
        ig_dict["body_news"].append(body_new)
        ig_dict["link"].append(url_news)
        ig_dict["category"].append(category_news)
        ig_dict["image"].append(image_new)
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingIgQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingIgQueueDTO(body.get('url'))
