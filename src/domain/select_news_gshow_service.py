...
class SelectNewsGShowService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News O Globo Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://gshow.globo.com/rss/gshow/"
        
        try:
            links_filtered = Utils.extract_links_from_rss_oglobo(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site gshow | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingOGloboQueueDTO = VoxradarNewsScrappingOGloboQueueDTO(url)
