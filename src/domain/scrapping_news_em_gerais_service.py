...
class ScrappingNewsEmGeraisService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Em Gerais Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        em_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_emgerais_queue_dto:VoxradarNewsScrappingEmGeraisQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_emgerais_queue_dto.url
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingEmGeraisQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingEmGeraisQueueDTO(body.get('url'))
