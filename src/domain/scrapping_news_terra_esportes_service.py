...
from src.types.voxradar_news_scrapping_terra_esportes_queue_dto import VoxradarNewsScrappingTerraEsportesQueueDTO
...
class ScrappingNewsTerraEsportesService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Terra Eleicoes Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        terra_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_terra_esportes_queue_dto:VoxradarNewsScrappingTerraEsportesQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_terra_esportes_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Terra Esportes: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Terra Esportes: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Terra Esportes: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Terra Esportes: {url_news} | {e}")     
            image_new = ""
        #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingTerraEsportesQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingTerraEsportesQueueDTO(body.get('url'))
