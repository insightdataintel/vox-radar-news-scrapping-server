...
from src.types.voxradar_news_scrapping_campo_grande_news_queue_dto import VoxradarNewsScrappingCampoGrandeNewsQueueDTO
...
class ScrappingNewsCampoGrandeNewsService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Campo Grande News Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        campograndenews_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_campograndenews_queue_dto:VoxradarNewsScrappingCampoGrandeNewsQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_campograndenews_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Campo Grande News: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            date = str(date).split('datePublished":')[1].split(',')[0].split(',')[0].split('<span')[0].replace('T', ' ').replace('Z', '').replace('"','').replace('h',':').replace('-04:00','').split('+')[0].strip()
            date = date + '-03:00'
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar a data da notícia do Campo Grande News: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            mode = ['div']
            classk = ['Texto clearfix bg-content-transp']
            paragraf = ['p']
            body_new = ''
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Campo Grande News: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Campo Grande News: {url_news} | {e}")     
            image_new = ""   
        #
        #
        campograndenews_dict["title"].append(title)
        campograndenews_dict["domain"].append(domain)
        campograndenews_dict["source"].append(source)
        campograndenews_dict["date"].append(date)
        campograndenews_dict["body_news"].append(body_new)
        campograndenews_dict["link"].append(url_news)
        campograndenews_dict["category"].append(category_news)
        campograndenews_dict["image"].append(image_new)
        

        print(campograndenews_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingCampoGrandeNewsQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingCampoGrandeNewsQueueDTO(body.get('url'))
