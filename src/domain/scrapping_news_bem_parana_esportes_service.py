...
from src.types.voxradar_news_scrapping_bem_parana_esportes_queue_dto import VoxradarNewsScrappingBemParanaEsportesQueueDTO
...
class ScrappingNewsBemParanaEsportesService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Bem Parana Esportes Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        bemparana_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_bemparana_queue_dto:VoxradarNewsScrappingBemParanaEsportesQueueDTO = self.__parse_body(body)
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Bem Parana Esportes: {url_news} | {e}")     
            title = ""  
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Bem Parana Esportes: {url_news} | {e}")
            date = ""  
    #
    #Pick body's news
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Bem Parana Esportes: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Bem Parana Esportes: {url_news} | {e}")     
            image_new = ""       
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingBemParanaEsportesQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingBemParanaEsportesQueueDTO(body.get('url'))
