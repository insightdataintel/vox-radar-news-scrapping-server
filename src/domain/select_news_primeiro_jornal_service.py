...
class SelectNewsPrimeiroJornalService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Primeiro Jornal | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://noticias.primeirojornal.com.br/"
        
        try:
            links_filtered = Utils.extract_links_from_page_primeirojornal(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site primeiro jornal | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingPrimeiroJornalQueueDTO = VoxradarNewsScrappingPrimeiroJornalQueueDTO(url)
