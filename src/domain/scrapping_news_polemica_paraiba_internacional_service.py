...
from src.types.voxradar_news_scrapping_polemica_paraiba_internacional_queue_dto import VoxradarNewsScrappingPolemicaParaibaInternacionalQueueDTO
...
class ScrappingNewsPolemicaParaibaInternacionalService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Polemica Paraiba Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        polemicaparaiba_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_polemicaparaiba_queue_dto:VoxradarNewsScrappingPolemicaParaibaInternacionalQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_polemicaparaiba_queue_dto.url
    ...
    #
        try:
            ...
            title = str(title).split("<title>")[1].split("- Polêmica Paraíba")[0].split('itemprop=')[0].split('- ISTOÉ DINHEIRO')[0].split('| Exame')[0].replace('- @aredacao','').strip()
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar o título da notícia do Polemica Paraiba Internacional: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Polemica Paraiba Internacional: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Polemica Paraiba Internacional: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Polemica Paraiba Internacional: {url_news} | {e}")     
            image_new = "" # #
        # #
        ...
        #
        #
        polemicaparaiba_dict["title"].append(title)
        polemicaparaiba_dict["domain"].append(domain)
        polemicaparaiba_dict["source"].append(source)
        polemicaparaiba_dict["date"].append(date)
        polemicaparaiba_dict["body_news"].append(body_new)
        polemicaparaiba_dict["link"].append(url_news)
        polemicaparaiba_dict["category"].append(category_news)
        polemicaparaiba_dict["image"].append(image_new)
        

        print(polemicaparaiba_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingPolemicaParaibaInternacionalQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingPolemicaParaibaInternacionalQueueDTO(body.get('url'))
