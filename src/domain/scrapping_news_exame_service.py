...
from src.types.voxradar_news_scrapping_exame_queue_dto import VoxradarNewsScrappingExameQueueDTO
...
class ScrappingNewsExameService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Exame Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        exame_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_exame_queue_dto:VoxradarNewsScrappingExameQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_exame_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Exame: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Exame: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            body_new = body_new.encode('latin-1', 'ignore').decode("utf-8",'ignore')

        except Exception as e:
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Exame: {url_news} | {e}")
            body_new= ""    
    #    
    # Pick category news
    #   
        category_news = soup.find_all("script", type="application/ld+json")
        category_news = str(category_news).split('articleSection":')[1].split(',')[0].replace('"','')
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Exame: {url_news} | {e}")     
            image_new = "" 
        # # #
        # #
        ...
        #
        #
        exame_dict["title"].append(title)
        exame_dict["domain"].append(domain)
        exame_dict["source"].append(source)
        exame_dict["date"].append(date)
        exame_dict["body_news"].append(body_new)
        exame_dict["link"].append(url_news)
        exame_dict["category"].append(category_news)
        exame_dict["image"].append(image_new)
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingExameQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingExameQueueDTO(body.get('url'))
