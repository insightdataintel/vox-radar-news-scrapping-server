...
from src.types.voxradar_news_scrapping_poliarquia_queue_dto import VoxradarNewsScrappingPoliarquiaQueueDTO
...
class ScrappingNewsPoliarquiaService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        ...
        poliarquia_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_poliarquia_queue_dto:VoxradarNewsScrappingPoliarquiaQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_poliarquia_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Poliarquia: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Poliarquia: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Poliarquia: {url_news} | {e}")
            return ReturnService(False, 'Did not collect the body of the News')    
    #    
    # Pick category news
    #   
        category_news = soup.find_all('span',itemprop= 'name')[1]
        category_news = str(category_news).split('">')[1].split('</')[0].lower()
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Poliarquia: {url_news} | {e}")     
            image_new = "" #
        # #
        ...
        #
        #
        poliarquia_dict["title"].append(title)
        poliarquia_dict["domain"].append(domain)
        poliarquia_dict["source"].append(source)
        poliarquia_dict["date"].append(date)
        poliarquia_dict["body_news"].append(body_new)
        poliarquia_dict["link"].append(url_news)
        poliarquia_dict["category"].append(category_news)
        poliarquia_dict["image"].append(image_new)
        

        print(poliarquia_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingPoliarquiaQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingPoliarquiaQueueDTO(body.get('url'))
