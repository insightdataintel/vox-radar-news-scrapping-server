...
from src.types.voxradar_news_scrapping_money_times_queue_dto import VoxradarNewsScrappingMoneyTimesQueueDTO
...
class ScrappingNewsMoneyTimesService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Money Times Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        moneytimes_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_money_times_queue_dto:VoxradarNewsScrappingMoneyTimesQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_money_times_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Money Times: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Money Times: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Money Times: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Money Times: {url_news} | {e}")     
            image_new = "" 
        # #
        # #
        ...
        #
        #
        moneytimes_dict["title"].append(title)
        moneytimes_dict["domain"].append(domain)
        moneytimes_dict["source"].append(source)
        moneytimes_dict["date"].append(date)
        moneytimes_dict["body_news"].append(body_new)
        moneytimes_dict["link"].append(url_news)
        moneytimes_dict["category"].append(category_news)
        moneytimes_dict["image"].append(image_new)
        

        print(moneytimes_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingMoneyTimesQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingMoneyTimesQueueDTO(body.get('url'))
