...
class SelectNewsPoliticalivreBrasilService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        ...
        url = "https://politicalivre.com.br/category/brasil"
        
        try:
            links_filtered = Utils.extract_links_from_page_politicalivre(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site politicalivre brasil | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingPoliticalivreBrasilQueueDTO = VoxradarNewsScrappingPoliticalivreBrasilQueueDTO(url)
