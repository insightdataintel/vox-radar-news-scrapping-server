...
class SelectNewsBrasil247Service(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Brasil247 | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site brasil247 | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingBrasil247QueueDTO = VoxradarNewsScrappingBrasil247QueueDTO(url)
