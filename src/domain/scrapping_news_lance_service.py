...
from src.types.voxradar_news_scrapping_lance_queue_dto import VoxradarNewsScrappingLanceQueueDTO
...
class ScrappingNewsLanceService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Ndmais Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        lance_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_lance_queue_dto:VoxradarNewsScrappingLanceQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_lance_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Lance: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Lance: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            no_text = ['Cartola','Leia outras','podcast','Foto','clique aqui','Assine o Premiere','VÍDEOS:','o app do Yahoo Mail','Assine agora a newsletter','via Getty Images']
            for x in body_news:
                for item in no_text:
                    if item in x:
                        x = ''
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Lance: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Lance: {url_news} | {e}")     
            image_new = "" 
        # #
        # #
        ...
        #
        #
        lance_dict["title"].append(title)
        lance_dict["domain"].append(domain)
        lance_dict["source"].append(source)
        lance_dict["date"].append(date)
        lance_dict["body_news"].append(body_new)
        lance_dict["link"].append(url_news)
        lance_dict["category"].append(category_news)
        lance_dict["image"].append(image_new)
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingLanceQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingLanceQueueDTO(body.get('url'))
