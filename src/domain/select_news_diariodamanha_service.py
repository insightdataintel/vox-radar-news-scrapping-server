...
class SelectNewsDiariodaManhaService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Diario da Manhã Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.dm.com.br/ultimas-noticias/"

        
        try:
            links_filtered = Utils.extract_links_from_page_diariodamanha(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site diariodamanha | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingDiariodaManhaQueueDTO = VoxradarNewsScrappingDiariodaManhaQueueDTO(url)
