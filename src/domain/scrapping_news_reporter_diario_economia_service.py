...
from src.types.voxradar_news_scrapping_reporter_diario_economia_queue_dto import VoxradarNewsScrappingReporterDiarioEconomiaQueueDTO
...
class ScrappingNewsReporterDiarioEconomiaService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Reporter Diario Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        reporterdiario_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_reporterdiario_queue_dto:VoxradarNewsScrappingReporterDiarioEconomiaQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_reporterdiario_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Reporter Diario Economia: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Reporter Diario Economia: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Reporter Diario Economia: {url_news} | {e}")
    ...
    # Pick image from news
        ...
        #
        #
        reporterdiario_dict["title"].append(title)
        reporterdiario_dict["domain"].append(domain)
        reporterdiario_dict["source"].append(source)
        reporterdiario_dict["date"].append(date)
        reporterdiario_dict["body_news"].append(body_new)
        reporterdiario_dict["link"].append(url_news)
        reporterdiario_dict["category"].append(category_news)
        reporterdiario_dict["image"].append(image_new)
        

        print(reporterdiario_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingReporterDiarioEconomiaQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingReporterDiarioEconomiaQueueDTO(body.get('url'))
