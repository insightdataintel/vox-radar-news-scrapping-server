...
class SelectNewsR7AgronegociosService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News R7 Agronegocios Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site r7 agronegocios | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingR7AgronegociosQueueDTO = VoxradarNewsScrappingR7AgronegociosQueueDTO(url)
