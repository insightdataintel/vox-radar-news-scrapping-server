...
class SelectNewsCNNBrasilNacionalService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News CNN Brasil | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.cnnbrasil.com.br/nacional/"
        
        try:
            links_filtered = Utils.extract_links_from_page_cnnbrasil(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site cnn brasil nacional | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingCNNBrasilNacionalQueueDTO = VoxradarNewsScrappingCNNBrasilNacionalQueueDTO(url)
