...
from src.types.voxradar_news_scrapping_brasil247_queue_dto import VoxradarNewsScrappingBrasil247QueueDTO
...
class ScrappingNewsBrasil247Service(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Brasil 247 Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        brasil247_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_brasil247_queue_dto:VoxradarNewsScrappingBrasil247QueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_brasil247_queue_dto.url
    ...
    #   
        ...
        category_news = str(category_news).split("pageCategory': '")[1].split("'")[0].replace('"','').lower()
    ...
    # Pick image from news
        ...
        brasil247_dict["domain"].append(domain)
        brasil247_dict["source"].append(source)
        brasil247_dict["date"].append(date)
        brasil247_dict["body_news"].append(body_new)
        brasil247_dict["link"].append(url_news)
        brasil247_dict["category"].append(category_news)
        brasil247_dict["image"].append(image_new)
        

        print(brasil247_dict)

        self.__send_queue(title, domain, source, body_new, date, category_news, image_new, url_news)

        return ReturnService(True, 'Sucess')

    def __parse_body(self, body:str) -> VoxradarNewsScrappingBrasil247QueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingBrasil247QueueDTO(body.get('url'))
