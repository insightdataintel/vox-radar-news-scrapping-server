...
from src.types.voxradar_news_scrapping_r7_agronegocios_queue_dto import VoxradarNewsScrappingR7AgronegociosQueueDTO
...
class ScrappingNewsR7AgronegociosService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News R7 Agronegocios Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        r7_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_r7_agronegocios_queue_dto:VoxradarNewsScrappingR7AgronegociosQueueDTO = self.__parse_body(body)
        url_news = voxradar_news_scrapping_r7_agronegocios_queue_dto.url
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do R7 agronegocios: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #   
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do R7 agronegocios: {url_news} | {e}")
            date = ""    
        #
    #Pick body's news
    #
            ...
            for i in range(0,len(mode)):
                for j in range(0,len(classk)):
                    try:
                        ...
                        self.logger.error(f"Não foi possível encontrar o corpo da notícia do R7 agronegocios: {url_news} | {e}")
                        body_new = ""    
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do R7 agronegocios: {url_news} | {e}")     
            image_new = ""
        #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingR7AgronegociosQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingR7AgronegociosQueueDTO(body.get('url'))
