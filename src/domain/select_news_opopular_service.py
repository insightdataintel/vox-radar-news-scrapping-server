...
class SelectNewsOPopularService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News O Popular Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site opopular | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingOPopularQueueDTO = VoxradarNewsScrappingOPopularQueueDTO(url)
