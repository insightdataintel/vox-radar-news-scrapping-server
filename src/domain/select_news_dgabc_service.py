...
class SelectNewsDgabcService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News Dgabc | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://www.dgabc.com.br/Servicos/RSS/Home"
        
        try:
            links_filtered = Utils.extract_links_from_rss_dgabc(url)
        except Exception as e:
            self.logger.error(f"Não foi possível encontrar os Links na página inicial do site dgabc | {e}")
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingDgabcQueueDTO = VoxradarNewsScrappingDgabcQueueDTO(url)
