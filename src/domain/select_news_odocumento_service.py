...
class SelectNewsODocumentoService(BaseService):
    sqs: Sqs
    ...
    def exec(self) -> ReturnService:
        self.logger.info(f'\n----- Select News O Documento | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        ...
        url = "https://odocumento.com.br/latest-news/"
    ...
    def __send_queue(self, url:str):
        message_queue:VoxradarNewsScrappingODocumentoQueueDTO = VoxradarNewsScrappingODocumentoQueueDTO(url)
