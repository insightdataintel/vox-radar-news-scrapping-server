...
from src.types.voxradar_news_scrapping_yahoo_financas_queue_dto import VoxradarNewsScrappingYahooFinancasQueueDTO
...
class ScrappingNewsYahooFinancasService(BaseService):
    ...
    def exec(self, body:str) -> ReturnService:
        self.logger.info(f'\n----- Scrapping News Yahoo Noticias Service | Init - {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S %z")} -----\n')
        yahoo_dict = {'title': [], 'domain':[],'source':[],'date': [], 'body_news': [], 'link': [],'category': [],'image': []}
        voxradar_news_scrapping_yahoo_financas_queue_dto:VoxradarNewsScrappingYahooFinancasQueueDTO = self.__parse_body(body)
    ...
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o título da notícia do Yahoo Financas: {url_news} | {e}")     
            title = ""
    #
    #Stardandizing Date
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar a data da notícia do Yahoo Financas: {url_news} | {e}")
            date = ""    
    #
    #Pick body's news
    #
    #
        try:
            ...
            self.logger.error(f"Não foi possível encontrar o corpo da notícia do Yahoo Financas: {url_news} | {e}")
    ...
    # 
        ...
        category_news = category_news.split('br.')[1].split('.yahoo')[0]
    ...
    # Pick image from news
        ...
        try:
            ...
            self.logger.error(f"Não foi possível encontrar imagens da notícia do Yahoo Financas: {url_news} | {e}")     
            image_new = "" #
        # #
    ...
    def __parse_body(self, body:str) -> VoxradarNewsScrappingYahooFinancasQueueDTO:
        body = json.loads(body)
        return VoxradarNewsScrappingYahooFinancasQueueDTO(body.get('url'))
